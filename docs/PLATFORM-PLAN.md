...
**Status counts:** 87 Done · total 87

*Generated 2026-09-14T04:19:01.716Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.32, 2026-09-13):** §16.7 = live status: **87 items Done — AI Tool Orchestrator shipped (free-first, config-driven).** **PX-AI-001 (mod-ai-orchestrator)**: every AI capability the platform runs or sells — product recommendations, image processing, internal data analysis, data cleaning, reporting, assistant turns — is routed by **configuration only**. Priority law encoded as data: **always-free providers first; paid models unlock strictly for paying customers** (billing-tier constitutional gate — even a premium tenant-override rule cannot push a free-tier customer onto a paid model). The pack holds: providers (free + paid classes) with per-task quality maps and **bitemporal pricing** (price/quality changes are new rows, routing reconstructs at any T incl. knowledge-time), per-task capability requirements, priority-merged decision-table routing directives (per task / tier / tenant / customer), monthly budgets with degrade-to-free-or-reject actions, rate-limit fallback chains, billing tiers with markup (free = platform absorbs cost; growth +25%, enterprise +10% margin), **BYOK KMS credential POINTERS (never secrets in routing payloads)**. Model adapters are host-injected ports — the kernel application imports zero provider SDKs (Total Agnosticism). Sold as a product in-platform and to tenants/clients; per-tenant customization via ConfigOverride, per-client policy via rules. Gateway surface: /ai/route + /ai/usage/:tenant (deep-resolved ctx map literals — nested config objects now supported by the arg engine). Live-verified three policy dimensions: free tier → $0-only chain · growth image-processing → paid model w/ KMS ref · enterprise internal data-cleaning → local self-hosted free (priority-85 rule). 11/11 orchestrator tests, **416/416 total, 0 skipped**, lint, typecheck, register-consistency green.
