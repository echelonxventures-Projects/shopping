...
**Status counts:** 82 Done · total 82

*Generated 2026-09-11T16:13:33.122Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.29, 2026-09-11):** §16.7 = live status: **82 items Done — production data plane + real cluster deploy shipped.** **PostgreSQL wire engine** (P3-STR-001, kernel/storage-pg): the production data plane — pg-class driver Reference Pack over the wire protocol; JSONB bitemporal records with partial current-version indexes, real ACID transactions, in-SQL asOf windows, transactional supersede, optimistic concurrency, durability-across-instances, pool sizing from config; lazy admission-safe migrations; admitted through the SAME conformance matrix as every engine against a real postgres:17 (now a CI service — the matrix runs on every push). **Dual-agnosticism proof now spans FOUR engines** — memory/file/sqlite/wire-PG — with identical golden-commerce economics ($90/$7) on the wire server: the agnosticism contract holds end-to-end from toy engine to production engine. **Real cluster deploy** (PX-INF-004, npm run deploy): composes topology from the live registry (42 products), builds the platform image (Dockerfile.deploy), imports into k3s containerd, applies pack-sized Deployment + Service — **verified live on colima k3s (v1.35): 2/2 pods Running, full shopper journey through the cluster Service** (register → cart $50 → checkout saga reserve+authorize+capture, $5 commission); deploy config is pack data (infra-composer deploy section). En route: colima k3s datastore recovered via clean server-dir reset; CI fixed (was pinned to Node 20 + master-only — now Node 26 + main). **398/398 tests, 0 skipped, lint, typecheck, register-consistency green.** Remaining Open rows: external/vendor business work only.
