...
**Status counts:** 83 Done · total 83

*Generated 2026-09-12T10:50:07.079Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.30, 2026-09-11):** §16.7 = live status: **83 items Done — deploy tooling made agnostic (your doctrine challenge, accepted).** The question "why is Docker hardcoded?" was RIGHT: `scripts/deploy.ts` and the README named docker/colima/k3s in **code** — a Doctrine-6 violation in my own last wave. Fixed: runtime tooling is now a **`runtimeAdapters` Reference-Pack section in the infra pack** — three adapters declared as ordered step lists of command templates (`docker-colima-class`: build→save→copy→node-import→apply→rollout · `podman-kind-class`: save→node-load→apply→rollout · `registry-class`: apply→rollout only). **`InfraComposer.deployPlan()` (product API) resolves the plan**; `scripts/deploy.ts` is an invariant executor containing **zero technology names** (spawn-only, tilde-expansion mechanics, stdin-fed manifests); swapping = `AETHER_DEPLOY_ADAPTER` env or pack `selected` — verified: 3 distinct plans resolved from data alone, unit-tested, unknown adapter rejected with the pack-data message. Re-deployed **through the agnostic executor onto the live k3s cluster: 2/2 rollout, health verified**. README/docs now speak in `-class` language with instance files named as such. 399/399 tests, 0 skipped, lint, typecheck, register-consistency green. Lesson recorded: agnosticism applies to the platform's own tooling, not just its domains. Remaining Open rows: external/vendor business work only.
