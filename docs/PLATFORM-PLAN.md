...
**Status counts:** 59 Done · total 59

*Generated 2026-09-08T13:20:51.184Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.18, 2026-09-08):** §16.7 = live status: **59 items Done — Phase-4 product wave complete.** New: **AI Commerce** (P4-AI-001): visual search over a swappable EmbeddingAdapter SPI (similarity thresholds/topK from pack; Reference Pack adapter swap proven with zero core change) + guarded shopping assistant — pack-defined boundaries (no direct payments, no regulated advice), tool grants (catalog.search/compare/cart.add/order.track), escalation triggers (complaint→human), session turn limits, EU AI Act limited-risk classification with mandatory AI disclosure (8/8). **Compliance Evidence Engine** (P4-AUD-001): SOC 2 TSC (CC6.1/CC6.7/CC7.2/CC8.1/PI3.1) + ISO 27001 (A.8.24/A.5.15/A.8.15) controls mapped to platform-module proof artifacts via a swappable EvidencePort; 90-day freshness windows, gap naming, readiness gating (threshold 3), workpaper exports (6/6). **Mobile Release Ops** (P4-MOB-001): phased rollout 1→5→20→50→100% with 24h hold gates, forced-upgrade floors w/ 14-day grace → hard block, OS minimums (iOS 16/Android 10), deep-link domain+route resolution to modules, push throttling (8/day, quiet hours, collapse keys) (5/5). 301/301 tests, lint (product law), typecheck, register-consistency all green. All four Phase-4 §16.5 product rows Done. Remaining: production hardening (real RDBMS/search adapters via conformance, IaC prod baseline, HA deploy, load-to-250k) — the Phase-6 §7 items that require infrastructure decisions.
