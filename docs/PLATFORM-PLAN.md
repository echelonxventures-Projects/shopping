...
**Status counts:** 80 Done · total 80

*Generated 2026-09-11T01:21:36.601Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.28, 2026-09-10):** §16.7 = live status: **80 items Done — single-click self-hosting shipped.** **One-command install** (PX-OPS-004): `npm start` = Node check → install-if-needed → boot → serve (verified: health 200 + register 200 with zero manual steps). **Docker single-command**: `docker compose up` → healthy container (healthcheck green), full API reachable from host through mapped port; `AETHER_BIND_HOST` env overrides the pack default for container networking (pack remains local source of truth). README quickstart: one command + 60-second test drive (register → browse → cart → checkout saga). Fixed en route: start.mjs spawn path bug (CJS resolution — now absolute ESM path). **393/393 tests, lint, typecheck, register-consistency green.** Next: wire-protocol SQL engine pack + cluster deploy via infra-composer (both pack swaps).
