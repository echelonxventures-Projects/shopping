...
**Status counts:** 43 Done · total 43

*Generated 2026-09-07T05:34:44.821Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.13, 2026-09-06):** §16.7 = live status: **43 items Done**. New: **Crypto Vault** (P1-SEC-001 / §4.5 wiring): field-level envelope encryption per tenant (KEK→DEK w/ HKDF derivation, TTL-cached DEKs), searchable blind indexes for PII exact-lookup, **tenant crypto-isolation proven** (cross-tenant decrypt fails on GCM auth tag), CBOM + PQ-readiness generated from scheme registry, crypto-agility swap via pack, constitutional key-bits floor enforced — 7/7. **Golden Financial Scenarios** (P1-E2E-002 / §11): G1 multi-vendor split hand-computed (70 → 36.00/27.00 payouts, 7.00 commission, PSP 70, ledger sum-to-zero), G2 tri-market tax modes (US excl / EU incl + facilitator / IN GST), G3 partial refund w/ pro-rated commission reversal + **balanced double-entry refund postings** (a first draft had unbalanced entries — the ledger invariant caught it, exactly what §11 exists for), G4 refurbished grading 85%−5% + watch-tier 5% reserves + serial-returner flag, G5 oversell=0 under 100-race stress — 5/5. 218/218 tests × 3 runs, lint (product law), typecheck green. **Phase 1 §3 capability buildout complete.** Next per sequence: Phase-2 market packs (markets-as-data wave + residency cells), Phase-3 scale proofs.
