...
**Status counts:** 60 Done · total 60

*Generated 2026-09-08T13:37:13.787Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.19, 2026-09-08):** §16.7 = live status: **60 items Done — real relational storage shipped.** New: **SQL Storage Engine** (PX-INF-001, `kernel/storage-sql`): a genuine relational engine over node:sqlite (embedded SQL, zero deps — the PostgreSQL-class Reference Pack path) implementing the Storage SPI with real ACID tables, prepared statements, in-SQL bitemporal `asOf` queries (window clauses in WHERE, not post-filtering), ordered full history, transactional closeVersion, on-disk durability across instances; **admitted through the SAME conformance matrix as every engine** (5/5). Dual-agnosticism Proof B extended to **three engines** — golden commerce economics (payable 90.00 / tax 7.00) now proven identical across memory, file, and SQL. 306/306 tests, lint (product law), typecheck, register-consistency green. Remaining for production: IaC prod baseline (infra/), HA deploy, 250k/hr load validation, and the wire-protocol Postgres adapter (same SPI — swap is config).
