...
**Status counts:** 78 Done · total 78

*Generated 2026-09-09T21:54:13.661Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.26, 2026-09-09):** §16.7 = live status: **78 items Done — full commerce surface live over HTTP.** **Gateway expansion** (PX-CTR-003): checkout saga, orders lifecycle, and the complete monetization loop now mounted behind the model-driven gateway — **23 endpoints total**. Live-verified via curl: **checkout saga** (authorize→reserve→capture→commit with per-step trace, sub-orders w/ commission, compensations empty, **idempotent replay returns the same order — no double charge**); **orders lifecycle** (place/get/transition with illegal hops rejected by the pack workflow message / bitemporal history with validFrom-validTo windows); **monetization** (subscribe → meter → usage → invoice with quota explainability: `qty=3, included=10000, billableQty=0, within included quota`). **OpenAPI 3.1 generated FROM the route pack** (`GET /openapi.json`, public) — path params converted, per-route security + error docs; the API documentation *cannot drift* from the surface because it IS the pack. Demo-saga adapter pattern: routes flagged `demoSaga` in pack data call a host-supplied adapter (dummy PSP + auto-seeded stock) — production swaps the adapter, not the route. **381/381 tests (11/11 gateway), lint, typecheck, register-consistency green.** Remaining Open rows: external/vendor work only.
