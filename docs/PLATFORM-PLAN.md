...
| P0-CTR-001 | Contract layer: data (Protobuf-class), events (CloudEvents), APIs (OpenAPI/AsyncAPI), telemetry (OTel) | Contracts | — | Done ✓ | Contracts published; lint gate in CI |
| P0-CTR-002 | Runtime SPI (RuntimeTarget entity + adapter) | Contracts | 001 | Done ✓ | Second runtime target admitted via harness |
...
| P0-GOV-002 | **Constitution v1 bootstrap** (T-C tier, M-of-N quorum, ratification ceremony) | Governance | 001 | Done ✓ | Charter signed; amendment protocol executable |
| P0-GOV-003 | Constitutional crypto floors (§4.5 minimums, publish-time validation) | Governance | 001,011 | Done ✓ | Config below floor rejected at publish |
| P0-GOV-004 | Simulation harness for config packs | Governance | 005,006 | Done ✓ | Bad commission rule caught pre-publish |
| P0-SEC-001 | Crypto-agility registry (CryptoScheme entities, key hierarchy, floors) | Security | 002 | Done ✓ | Scheme swap via epoch; CBOM generated |
| P0-SEC-002 | IaC baseline + Reference Pack v1 (runtime/storage defaults as config) | Infra | 002,003 | Done ✓ | Full env from IaC; pack swap demo |
| P0-SEC-003 | CI pipeline v0 (test/lint/contract-compat/epoch-stamp/secret-scan) | Infra | 001 | Done ✓ | CI green gates on kernel packages |
...
| P1-CAT-003 | Type packs wave 2: perishables, regulated, CPQ, freight, used/C2C, auctions, rentals, trade-in, group-buy, live/social, AR/3D, voice, charity, fractional | Catalog | 002 | Done ✓ | Same |
| P1-CAT-004 | Health vertical pack (HIPAA-class, Rx, serialization) | Catalog | 002 | Done ✓ | Vertical compliance review passed |
...
| P1-UCA-001 | Unified commerce/POS (offline-first, RFID, BOPIS) | Fulfillment | LOG-001 | Done ✓ | In-store pickup/return E2E |
| P1-SUP-001 | Support portal + decision-explainability | Support | ORD-001,RUL-001 | Done ✓ | "Why this charge" per line |
| P1-FIN-001 | Tenant/seller finance portal (P&L, settlements, BI) | Finance | MON-001 | Done ✓ | Settlement reports match ledger |
...
| P1-SEC-002 | PCI SAQ-A evidence + SOC 2 evidence collection | Security | P1-SEC-001 | Done ✓ | Continuous evidence pipeline live |
...
|---|---|---|---|---|
| P0-CTR-001 | Contract layer — CloudEvents-class event envelopes + OTel-class/W3C telemetry + contract-compatibility gate, all GENERATED from registry data in kernel/codegen (no hand-written per-service event or span shape) | CTR | Done | codegen tests 7/7: envelope required-attrs + tenant/market/epoch extensions, malformed-event rejection, W3C traceparent + OTel span w/ registry attrs, contract-compat flags type-change/classification-tightening/newly-required as breaking |
| P0-CTR-002 | Runtime SPI — RuntimeTarget entity + RuntimeAdapter SPI in kernel/runtime-target; runtime adapters admitted ONLY by passing the generated runtime conformance matrix (deploy/scale/health/undeploy/capabilities/target-match) | CTR | Done | SECOND runtime target admitted via harness (container-orchestrator-class + process-host-class both admitted); broken adapters refused (no-scale, unhealthy-convergence); target-kind isolation enforced; 7/7 tests |
...
| P0-GOV-002 | Constitution v1 bootstrap — T-C tier, M-of-N quorum ratification, amendment protocol with automated verification + auto-revert (kernel/governance) | GOV | Done | governance tests: floors enforced, M-of-N amendment enactment, auto-revert on breach; delivered with P0-GOV-001 ConfigStore |
| P0-GOV-003 | Constitutional crypto floors — §4.5 minimums validated at publish time, floor violations rejected by the registry (kernel/governance floors + CryptoRegistry) | GOV | Done | weak-scheme registration rejected with /floor/ (AES-64 refused); floors enforced on every publish |
...
| P0-SEC-001 | Crypto-agility registry — CryptoScheme entities, key hierarchy, floors, CBOM + PQ readiness generated (kernel/governance + services/crypto-vault field-level envelope encryption) | SEC | Done | scheme swap via epoch; CBOM generated; PQ readiness ratio reported; crypto-vault 7/7 (cross-tenant decrypt fails cryptographically, floor violations rejected) |
| P0-SEC-002 | IaC baseline + Reference Pack v1 — runtime/storage defaults as config: infra-composer composes topology FROM the live product registry + pack shapes; runtimeAdapters make the deploy tooling itself swappable; Dockerfile/compose are container-class instance files | SEC | Done | full env from IaC (pack-sized Deployment+Service, live on k3s); pack swap demo (3 runtime adapters); topology auto-extends with every new product |
| P0-SEC-003 | CI pipeline v0 — lint (pack-sourced doctrine rules) + typecheck + full test suite + register projection staleness + register consistency + secret scan; postgres:17 service container for wire-engine conformance; Node 26 for native TS | SEC | Done | CI green gates on kernel packages + services; verified pipeline: 450 tests incl. conformance matrix; secret-scan rule in lintRulePack |
...
| P1-CAT-003 | Product type packs WAVE 2 — regulated, CPQ, freight/LTL, used/C2C, trade-in, auction, group-buy, live/social, AR-3D, voice, charity, fractional as pure pack data; mergeProductPacks() unions base+wave packs with no code changes | CAT | Done | 12 new types + 14 taxonomy nodes + 11 attribute sets + 2 identity schemes, all pack-only; multi-set required-attribute binding fixed (was first-match only); 20/20 product-master tests |
| P1-CAT-004 | Health vertical pack — HIPAA-class: PHI classification + retention + consent purpose, Rx rules (RxNorm/NDC/DEA schedule/prescriber/refill), medical device rules (UDI-DI/device class/sterilization/implantable), telehealth service type; NDC/RxNorm/UDI-DI/DEA identity schemes | CAT | Done | health-rx binds THREE required sets simultaneously (pharma+hipaa+rx); device UDI + HIPAA privacy attrs mandatory; pattern enforcement per scheme; 20/20 product-master tests |
...
| P1-FIN-001 | Tenant/seller finance portal — P&L from double-entry ledger feed with pack prefix-map classification, PSP settlement reconciliation w/ tolerance + exception queue, GMV bucketing by pack period, per-seller statements; unclassified money surfaced (never dropped) | FIN | Done | settlement reports match ledger; amount/date exceptions queued + clearable; GMV by day/month/quarter/year from pack; 7/7 tests (incl. a real bug fix: unclassified entries were silently dropped) |
...
| P1-SEC-002 | PCI SAQ-A evidence + SOC 2 evidence collection — compliance-evidence product: framework control catalogs (SOC 2 TSC, ISO 27001) mapped to platform-module proof artifacts via swappable EvidencePort, freshness windows, gap alerts, readiness scoring, workpaper exports | SEC | Done | evidence collected continuously via port; gap detection names the missing module; freshness window (90d) breaches; readiness threshold from pack; SOC2+ISO27001 catalogs; 6/6 tests |
...
| P1-UCA-001 | Unified commerce/POS — register sessions w/ float reconciliation + variance tolerance, pack-defined RFID/barcode scan formats + dedupe window, tender/change rules, BOPIS hold window + high-value ID verification, offline-first queue w/ price-drift protection | UCA | Done | in-store pickup/return E2E: session open→sale→reconcile; RFID EPC + EAN13 + staff-badge parsed from pack; card-overpay refused, cash change returned; BOPIS expiry + ID gate; offline drift HOLDS for review; 10/10 tests |
...
| P3-STR-001 | PostgreSQL wire-protocol storage engine — production data plane (pg-class driver Reference Pack): JSONB bitemporal records, real transactions, in-SQL asOf windows, transactional supersede, partial indexes, pool sizing from config; lazy admission-safe migration; AETHER_PG_DSN config. CI runs the full conformance matrix against a real postgres:17 service. Dual-agnosticism proof extended: FOUR engines (memory/file/sqlite/wire-PG) — identical golden commerce economics on the wire engine | STR | Done | wire engine ADMITTED through conformance against live server; asOf/history/supersede/optimistic-concurrency/durability-across-instances proven over the wire; golden chain equivalence $90/$7 on PG = memory = file = sqlite; CI postgres:17 service + AETHER_PG_DSN; 398/398 with 0 skips |
...
| PX-AI-001 | AI Tool Orchestrator — free-first model routing as pure configuration: providers/prices/quality (bitemporal), per-task capability requirements, routing decision-tables (priority per task/tenant/tier/customer), constitutional billing-tier gate (free NEVER gets paid models), budget guard (degrade-to-free vs reject), fallback chains, markup-based tenant billing with platform absorption, BYOK KMS credential POINTERS, host-injected model adapters (zero SDK imports). Sold as a product in-platform + to tenants/clients | AI | Done | 11/11 orchestrator tests: free-first law, paid quality unlock, internal-ops-stay-free rule, bitemporal price reconstruction incl. knowledge time, constitutional tier gate beats tenant override, budget degrade, rate-limit fallback, markup billing (free absorbs, growth +25%), BYOK pointer-only; live /ai/route: free→$0 chain-only, growth images→paid+KMS ref, enterprise data-cleaning→local free |
| PX-CTR-002 | API Gateway (model-driven HTTP surface) — every route, auth scope, demo key, rate limit, CORS binding is PACK DATA (method+path -> module apiMethod w/ JSON-path arg mapping); public /health self-describes; demo credentials ship in pack for local curl-testing; npm run serve boots the full platform behind it | CTR | Done | gateway tests 10/10 — route matching w/ :params, scope enforcement 401/403, JSON-path mapping + literal/fallback/coercion, rate-limit 429, self-describing /health, unmounted 503, handler errors 422; live curl E2E verified (search/tax/pricing/payments/geo) |
| PX-CTR-003 | Gateway expansion — checkout saga, orders lifecycle, monetization billing loop mounted over HTTP; OpenAPI 3.1 GENERATED from the route pack (docs cannot drift); demo-saga adapter pattern (dummy PSP + auto-seeded stock); idempotent replay verified | CTR | Done | live curl E2E: checkout saga w/ trace + compensations + idempotent replay; orders place/get/transition (illegal rejected w/ pack workflow msg)/bitemporal history; monetization subscribe->meter->usage->invoice w/ quota explainability; /openapi.json self-describes (11/11 gateway tests) |
| PX-CTR-004 | Shopper flow over HTTP — identity (register/login/sessions: scrypt-salted hashes, timing-safe compare, pack TTL policy) + session-scoped carts (pack caps, checkout consumes cart) + seeded shoppable catalog; gateway session-auth mechanism (Bearer tokens, $.session mappers, shopper scope); full journey live-verified: register -> browse -> add-to-cart -> checkout saga -> cart emptied | CTR | Done | identity 5/5 + cart 5/5 + gateway session tests 13/13; live curl: register returns cust_00001 + Bearer token; /auth/me resolves; cart totals correct; /shop/checkout saga reserves+authorizes+captures with commission fee; cart auto-empties |
...
| PX-HRD-001 | Edge security — gateway credential sources as pack data (demo-inline | secret-file KMS-mount | host-resolved injection); unknown mode rejected; TLS + HSTS via pack-declared env names (live-verified: https health + strict-transport-security header from pack maxAge) | HRD | Done | gateway tests 15/15 — demo key DEAD in secret-file mode, mounted key works, host resolver works; TLS verified with self-signed cert |
| PX-HRD-002 | Distributed shopper state — users, sessions, carts store-backed on Storage SPI (conformance-admitted engines; store-authoritative reads; stable cart record windows; upsert-safe), PostgresRateLimiter shared-window cluster-wide limiter wired into gateway | HRD | Done | cross-pod tests: register A→login B, cart A→take B; live 2-process proof (pod B resolves pod A session + cart $50 through wire engine); limiter unit-proven across instances |
| PX-HRD-003 | Durable search + real-cluster game day — StorageSearchEngine (mirrors writes to Storage SPI, rehydrates full index on fresh pod, identical ranking reused verbatim); ops-center runDrill async-harness upgrade; drill_pod_kill pack scenario executed against live k3s via scripts/gameday.ts | HRD | Done | search: writes mirror, pod-B rehydrate = full index, removal durable; GAME DAY LIVE: pod killed grace=0, self-heal 8s, replicas 2/2, service health 200 through ClusterIP, rollback verified, incident evidence written |
...
| PX-INF-004 | Real cluster deploy — npm run deploy: composes topology from live registry (42 products), builds platform image (Dockerfile.deploy), imports into k3s containerd, kubectl-applies pack-sized Deployment (readiness/liveness on /health, resource limits from sizing classes) + ClusterIP Service; deploy config is PACK DATA (infra-composer-core deploy section); verified live on colima k3s: 2/2 pods Running, full shopper journey through the cluster Service (register -> cart $50 -> checkout saga w/ commission) | INF | Done | rollout complete on real k8s (v1.35); port-forward to Service: /health 200 w/ 34 routes, in-cluster register cust_00002, cart total $50, checkout saga trace reserve+authorize+capture, fee $5; fresh-cluster CA/kubeconfig recovery documented in deploy mechanics |
| PX-INF-005 | Agnostic deploy tooling — Doctrine 6 applied to the deploy pipeline itself: runtime tooling is now runtimeAdapters Reference Packs in the infra pack (docker-colima-class / podman-kind-class / registry-class), each an ordered step list of command TEMPLATES; InfraComposer.deployPlan() resolves the plan (product API); scripts/deploy.ts is an invariant executor with ZERO tech names in code; adapter swap = AETHER_DEPLOY_ADAPTER env or pack selected entry, verified live (redeploy through executor onto real k3s) + unit proof of 3 distinct plans | INF | Done | infra-composer tests 7/7 — docker/podman/registry plans differ by steps+imageRef with zero code, unknown adapter rejected (pack-data message); live deploy via executor rolled out 2/2; README/skip-hints de-vendored (-class language); tilde expansion fix in executor |
...
| PX-OPS-004 | Single-click self-hosting — npm start bootstrapper (Node check -> install-if-needed -> boot -> serve) + Docker single-command (Dockerfile, docker compose up w/ healthcheck, AETHER_BIND_HOST env override of pack default) + README quickstart with 60-second test drive; container verified live (health 200, register 200 through mapped port) | OPS | Done | npm start boots to serving API with zero manual steps; docker compose up -> healthy container, registration + health 200 from host; 393/393 tests still green |
...
**Status counts:** 99 Done · total 99

*Generated 2026-09-16T16:23:39.132Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.33, 2026-09-16):** §16.7 = live status: **99 items Done — the buildable backlog is closed.** This wave finished every remaining in-repo item and reconciled shipped-but-stale rows with evidence. **Contract layer** (P0-CTR-001, kernel/codegen): CloudEvents 1.0 envelopes with tenant/market/epoch extensions + required-attribute validation, W3C traceparent + OTel-class spans carrying registry epoch attributes, and a **contract-compatibility gate** that classifies epoch-to-epoch schema changes (type-change, classification-tightening, newly-required = breaking; additive = compatible) — all generated from registry data, never hand-shaped. **Runtime SPI** (P0-CTR-002, kernel/runtime-target): RuntimeTarget as a registry entity with capability descriptors + RuntimeAdapter SPI, and a **generated runtime conformance matrix** (deploy/scale/health-convergence/undeploy/capabilities/target-match) — a SECOND runtime target is admitted through the harness (container-orchestrator-class AND process-host-class), while broken adapters (no-scale, lying-health) are refused; the harness itself caught a real undeploy-semantics bug in my adapters. **Product type packs WAVE 2** (P1-CAT-003): regulated, CPQ, freight/LTL, used/C2C, trade-in, auction, group-buy, live/social, AR-3D, voice, charity, fractional — 12 types, 14 taxonomy nodes, 11 attribute sets, 2 identity schemes, all pack-only, with `mergeProductPacks()` unioning base+wave packs. **Health vertical** (P1-CAT-004): HIPAA-class PHI classification/retention/consent, Rx rules (RxNorm/NDC/DEA schedule/prescriber/refill), device rules (UDI-DI/class/sterilization/implantable), telehealth type; health-rx binds **three** required attribute sets simultaneously. **Finance portal** (P1-FIN-001): pack-prefix-map P&L classification, PSP settlement reconciliation with tolerance + clearable exception queue, GMV bucketing, seller statements. **POS / unified commerce** (P1-UCA-001): register sessions with float reconciliation + variance tolerance, pack-defined RFID/barcode scan formats with dedupe windows, tender/change rules, BOPIS hold windows + high-value ID verification, offline queue with price-drift HOLDS. Reconciled with verified evidence: P0-GOV-002/003, P0-SEC-001/002/003, P1-SEC-002 (governance constitution+floors, crypto registry, IaC baseline, CI pipeline, compliance-evidence all shipped earlier). **450/450 tests, 0 skipped, build, lint, typecheck, register-consistency green.** Two real bugs found and fixed by this wave's tests: unclassified money was silently dropped from P&L statements, and multi-set required-attribute binding only applied the first set. **The 11 remaining Open rows are exclusively external-world work** — PSP platform agreements, vendor certifications, facilitator tax registrations, legal templates, content cold-start, regional carrier/COD operations, finance-ops staffing, docs site, pentest cadence — none buildable in-repo without your vendor decisions and contracts.
