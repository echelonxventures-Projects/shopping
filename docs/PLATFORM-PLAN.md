...
**Status counts:** 79 Done · total 79

*Generated 2026-09-10T07:49:10.989Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.27, 2026-09-10):** §16.7 = live status: **79 items Done — full shopper journey live (login → shop → checkout).** **Shopper flow** (PX-CTR-004): **Identity product** (services/identity) — register/login with scrypt-salted credential hashes + timing-safe compare (no plaintext at rest), session tokens with pack TTL, pack-declared password policy (length/digit/letter), email normalization + uniqueness (409) (5/5). **Cart product** (services/cart) — session-scoped carts with pack caps (maxLines, qty-per-line), qty-0-removes semantics, totals, `take()` consumes cart for checkout (5/5). **Gateway session-auth mechanism**: routes with `scope: "shopper"` authenticate via `Authorization: Bearer <token>` (dual-auth: api-key OR session); `$.session.customerId` + `$.session_token` + `$.cart_lines` mapper sources resolve the logged-in shopper and their cart; auth routes public. **Seeded shoppable catalog**: 3 products with offers + stock auto-seeded at server boot. Live-verified journey: register → `cust_00001` + token → `/auth/me` → search "aether" → add-to-cart (2× $25 = $50) → `/shop/checkout` saga (reserve→authorize→capture→notify, seller fee $5) → cart auto-emptied. **394/394 tests (13/13 gateway), lint, typecheck, register-consistency green.** Remaining Open rows: external/vendor work only.
