...
**Status counts:** 77 Done · total 77

*Generated 2026-09-09T16:14:12.135Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.25, 2026-09-09):** §16.7 = live status: **77 items Done — curl-able local API shipped.** **API Gateway** (PX-CTR-002, services/gateway): the platform's HTTP surface is 100% MODEL-DRIVEN — every route (method+path → module apiMethod), auth scope, demo API key, rate limit, and CORS binding is PACK DATA (`gateway-core.json`); adding an endpoint = a pack edit, never code. JSON-path argument mapping (`$.body.x`, `:params`, `?query`, literals, `||` fallbacks with numeric coercion), `argsMode` object/positional per route, public-route flag. Ships with **demo credentials** so the whole platform is curl-able locally via `npm run serve` (http://127.0.0.1:8080): `demo-admin-key-0000` (full access) / `demo-shopper-key-0000` (read+AI); public `GET /health` self-describes all routes; verified live: search, tax (US-CA 9.25% w/ explainability), pricing quote (promo stack $28→$20.20), payment authorize (SAQ-A token-only), geo zones (London congestion postal-prefix), scope enforcement (shopper 403 on catalog:write), rate-limit 429 (10/10 gateway tests; production keys land as KMS-backed pack swaps). **380/380 tests, lint, typecheck, register-consistency green.** Remaining Open rows: external/vendor work only.
