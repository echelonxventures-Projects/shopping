...
**Status counts:** 31 Done · total 31

*Generated 2026-09-06T16:50:26.021Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.9, 2026-09-06):** §16.7 = live status: **31 items Done**. New: **World-as-ECR** (P1-WLD-001): worlds are entities (Earth/Luna/Mars shipped; Europa added in tests via pure pack data), orbits/trade-lanes are relationships, `world` is a first-class context dimension in the kernel ContextFrame + resolver (all scoped config can vary per world: SLA 5 days vs 668-sol interplanetary, lunar-gravity weight display), calendars/units/SLA/physics are configuration, and the only invariant is the atomic-seconds spine — bitemporal + ledger math is world-agnostic while every world projects its own calendar/units over it (Mars: 88,775-second sols, sol-quoted SLAs, thin-atmosphere logistics constraints). 157/157 tests, lint, typecheck green. Pending from module wave: convert remaining services to module contracts (catalog done as spec, 9 to go); then P1-SUP/REC/EXP/B2B/ONB/SEO.
