...
**Status counts:** 37 Done · total 37

*Generated 2026-09-06T23:55:57.415Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.11, 2026-09-06):** §16.7 = live status: **37 items Done**. New: **Everything-is-a-Product law live** (P0-KRN-016 — lint-enforced: every service ships `module.json` + bundled packs + `export default XModule` + meterableEvents; `kernel/product-registry` auto-lists all products and generates Monetization Stack wiring). New products born as modules: **Support** (P1-SUP-001: ticket workflow, AI triage rules, SLA clocks, decision-explainability journal — 7/7), **Recommendations** (P1-REC-001: weighted retrieval, diversity/inventory-aware/sponsored-labeled re-rank, consent gate, explanations — 6/6), **Experimentation** (P1-EXP-001: A/B/n, deterministic assignment, significance-promotion, guardrail kill-switch — 5/5), **Two-Step Request Admission** (P1-ADM-001, user spec: Step 1 preflight non-binding w/ TTL grant, Step 2 finalize live re-check + atomic reserve; `BackendAdmissionRejected` exact contract `dp_rank=3, policies=prefill_pressure`; all thresholds pack data — 8/8). 188/188 tests, lint, typecheck green. Next per sequence: P1-B2B, P1-ONB-001 (migration), P1-SEO-001, notifications; then Phase-2 market packs.
