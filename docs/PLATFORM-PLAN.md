...
**Status counts:** 45 Done · total 45

*Generated 2026-09-07T07:45:59.870Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.14, 2026-09-06):** §16.7 = live status: **45 items Done — Phase 2 commenced.** New products: **Market Registry** (P2-MKT-001 / §3.2b): markets-as-data — 5 markets shipped (US/EU/IN/BR/AE) each as ONE pack entry (locales, currency, tax regime descriptor + e-invoicing adapter, payment rails incl. UPI/Pix/iDEAL/PayNow, compliance packs CCPA/GDPR/DPDP/LGPD/PDPA, capability flags COD/BNPL/DDP, residency cells, consumer-law windows, ID schemes); tenant×market activation matrix; **config-only onboarding proven** — Singapore added at runtime via one data object, zero code (7/7). **Residency Enforcement** (P2-MKT-002): market→cell routing with sovereignty regimes; cross-cell writes REFUSED with `ResidencyViolationError` (hard-fail); engine-per-cell (any admitted StorageEngine per cell); DSR tenant erasure per cell; full audit trail (5/5). 230/230 tests × 3 runs, lint (product law), typecheck green. Next per sequence: P2-MKT-003 (regional compliance pack wiring — DSR APIs, consent), P2-ECO-001 (app marketplace/dev sandboxes), P2-OFF-001 (offline mode).
