...
**Status counts:** 86 Done · total 86

*Generated 2026-09-13T03:06:22.807Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.31, 2026-09-13):** §16.7 = live status: **86 items Done — the production-hardening wave is complete.** All six known gaps closed, each pack-driven and live-verified: **(1) KMS-class credential sources** (PX-HRD-001): gateway `keyProvider` mode is pack data — demo-inline (dev), secret-file (KMS/Vault-agent mount, env-declared path; demo keys provably DEAD in production mode), host-resolved (external platform injects its own resolver); unknown mode rejected with the pack-data message. **(2) TLS + HSTS** (PX-HRD-001): cert/key env names + max-age read from the gateway pack; live-verified https health + `strict-transport-security: max-age=31536000; includeSubDomains`. **(3) Distributed shopper state** (PX-HRD-002): users, sessions AND carts now store-backed on the Storage SPI — store-authoritative reads, stable cart record windows, upsert-safe; **live two-process proof: register on pod A → login on pod B, cart on A consumed by B ($50 seen cross-pod)** — through the conformance-admitted wire engine. **(4) Cluster-wide rate limiting** (PX-HRD-002): PostgresRateLimiter shared-window counters; gateway `mountRateStore` (async-safe fix made the Promise-truthy bug impossible); unit-proven across instances. **(5) Durable search** (PX-HRD-003): StorageSearchEngine — write-mirroring + full rehydration on fresh pods, ranking logic reused verbatim (identical results contract); auto-selected when the host supplies storage. **(6) Game day on the real cluster** (PX-HRD-003): `drill_pod_kill` pack scenario + kubectl harness executed live — pod deleted grace=0, **self-healed in 8s**, replicas restored, service health 200 through the ClusterIP, rollback verified, incident evidence written. Ops-center `runDrill` upgraded to async harnesses (real-cluster probes). Cluster redeployed with the new code (2/2 Running, 34 routes, OpenAPI 200). Found + fixed en route: expired-session handler called `deleteAll()` (would've wiped every session); sync `rateOk` silently accepted async limiter. **405/405 tests, 0 skipped, lint, typecheck, register-consistency green.** Remaining: vendor/business decisions only (real PSP keys, cloud account, DNS/certs, audits).
