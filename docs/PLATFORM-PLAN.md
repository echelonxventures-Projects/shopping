...
**Status counts:** 75 Done · total 75

*Generated 2026-09-09T14:42:35.414Z by `npm run register:project` · source: packs/platform-program/{pack.json, work-items.jsonl}*
**Register status snapshot (Epoch 2.23, 2026-09-09):** §16.7 = live status: **75 items Done — cross-platform portability shipped.** **Portable Product Bundles** (PX-PRD-002, kernel/module `BundleExchange`): every registered module EXPORTS as a self-contained product bundle — manifest + FULL pack contents + host/billing contract notes — and INSTALLS into a *different* runtime/host, where it meters usage to the **buyer platform's own BillingPort**. Verified: platform A exports mod-logistics, platform B (external host + its own counting billing) installs and meters `label.generated` to its own ledger — the platform's products are literally sellable to other platforms as plug-and-play units. Smallest-value configurability proven in the same suite: one nested pack number (returnless-refund $5 → $7.25) host-overridden and tenant-scoped `windowDays=99` tenant-overridden via deep merge, sibling keys preserved (12/12 kernel/module tests). **358/358 tests, lint (product law + kernel-domain-ban), typecheck, build, register-consistency green.** Remaining Open rows are exclusively external/vendor work — no further code buildable in-repo without user vendor/cloud decisions.
